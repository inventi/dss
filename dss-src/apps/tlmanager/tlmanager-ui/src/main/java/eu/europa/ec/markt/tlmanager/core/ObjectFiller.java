...
public class ObjectFiller {
    ...
    /**
     * Convenience method for wrapping any <code>JAXBElement</code> into an <code>AnyType</code> object
     * 
     * @param <T> an arbitrary type
     * @param element a <code>JAXBElement</code> to be wrapped
     * @return an <code>AnyType</code> that contains the provided object
     */
    ...
    /**
     * Fills a given <code>TrustStatusListType</code> with all needed objects.
     * 
     * @param tsl the <code>TrustStatusListType</code>
     * 
     * @return the filled <code>TrustStatusListType</code>
     */
    public static TrustStatusListType fillAll(TrustStatusListType tsl) throws FillerException {
        try {
                        ...
                        for (TSPServiceType service : tsp.getTSPServices().getTSPService()) {
                            ...
                            if (serviceInformation != null) {
                                ...
                                if (service.getServiceHistory() != null) {
                                    for (ServiceHistoryInstanceType history : service.getServiceHistory()
            ...
            if (tsl.getSchemeInformation() != null && tsl.getSchemeInformation().getPointersToOtherTSL() != null) {
                for (OtherTSLPointerType pointer : tsl.getSchemeInformation().getPointersToOtherTSL()
                        .getOtherTSLPointer()) {
     ...
     * 
     * @param info the <code>TSLSchemeInformationType</code> to fill
     * 
     * @return the filled object
     */
    public static TSLSchemeInformationType fillTSLSchemeInformationType(TSLSchemeInformationType info)
        ...
        try {
            ...
            if (info.getSchemeOperatorAddress() == null) { // SchemeOperatorAddress type=tsl:AddressType
                ...
                if (info.getSchemeOperatorAddress().getElectronicAddress() == null) {
                    info.getSchemeOperatorAddress().setElectronicAddress(
                            objectFactoryTSL.createElectronicAddressType());
                }
                if (info.getSchemeOperatorAddress().getPostalAddresses() == null) {
                    info.getSchemeOperatorAddress().setPostalAddresses(
                            objectFactoryTSL.createPostalAddressListType());
                } else {
            ...
            if (info.getSchemeTypeCommunityRules() == null) { // tsl:SchemeTypeCommunityRules
                info.setSchemeTypeCommunityRules(objectFactoryTSL.createNonEmptyURIListType());
                info.getSchemeTypeCommunityRules().getURI();    // create the arraylist
            }

            if (info.getSchemeTerritory() == null || info.getSchemeTerritory().isEmpty()) { // tsl:SchemeTerritory
            ...
            if (info.getDistributionPoints() == null) { // tsl:DistributionPoints
                info.setDistributionPoints(objectFactoryTSL.createElectronicAddressType());
            }
        } catch (Exception ex) {
    ...
    public static OtherTSLPointerType fillOtherTSPPointerType(OtherTSLPointerType pointer) throws FillerException {
        try {
            ...
            if (!stcr) { // tsl:SchemeTypeCommunityRules
                NonEmptyURIListType nonEmptyURIListType = objectFactoryTSL.createNonEmptyURIListType();
                JAXBElement<NonEmptyURIListType> schemeTypeCommunityRules = objectFactoryTSL
                        .createSchemeTypeCommunityRules(nonEmptyURIListType);
            ...
            if (!tt) { // tsl:TSLType
                JAXBElement<String> TSLType = objectFactoryTSL.createTSLType(Configuration.getInstance().getTSL()
                        .getTslTypeInverse());
     ...
     * Ensure that all necessary fields are there to which a value might be bound
     * 
     * @param tspInformation the <code>TSPInformationType</code> to fill
     * 
     * @return the filled object
     */
    public static TSPInformationType fillTSPInformationType(TSPInformationType tspInformation)
            throws FillerException {
        try {
            ...
            if (tspInformation.getTSPAddress() == null) { // TSPAddress
                ...
                if (tspInformation.getTSPAddress().getElectronicAddress() == null) {
                    tspInformation.getTSPAddress().setElectronicAddress(
                            objectFactoryTSL.createElectronicAddressType());
                }
                if (tspInformation.getTSPAddress().getPostalAddresses() == null) {
                    tspInformation.getTSPAddress()
                            .setPostalAddresses(objectFactoryTSL.createPostalAddressListType());
                }
            }
     ...
     * Ensure that all necessary fields are there to which a value might be bound
     * 
     * @param serviceInfo the <code>TSPServiceInformationType</code> to fill
     * 
     * @return the filled object
     */
    public static TSPServiceInformationType fillTSPServiceInformationType(TSPServiceInformationType serviceInfo)
            throws FillerException {
        try {
     ...
     * Ensure that all necessary fields are there to which a value might be bound
     * 
     * @param historyInstance the <code>ServiceHistoryInstanceType</code> to fill
     * 
     * @return the filled object
     */
    public static ServiceHistoryInstanceType fillServiceHistoryInstanceType(
            ServiceHistoryInstanceType historyInstance) throws FillerException {
        try {
    ...
    private static AdditionalServiceInformationType fillAdditionalServiceInformationType(
            AdditionalServiceInformationType asi) throws FillerException {
        if (asi.getURI() == null) {
    ...
    public static QualificationElementType fillQualificationElement(QualificationElementType element)
        ...
        try {
            ...
            if (criteriaList.getPolicySet().isEmpty()) {
                ...
                // make sure, that all <ecc:PolicyIdentifier> are in the same <ecc:PolicySet>
                List<ObjectIdentifierType> shiftedPolicies = new ArrayList<ObjectIdentifierType>();
                List<PoliciesListType> policySet = criteriaList.getPolicySet();
                if (policySet.size() > 1) {
                    for (PoliciesListType policy: policySet) {
                        shiftedPolicies.addAll(policy.getPolicyIdentifier());
                    }
                    policySet.clear();
                    PoliciesListType polyListType = objectFactoryECC.createPoliciesListType();
                    polyListType.getPolicyIdentifier().addAll(shiftedPolicies);
                    policySet.add(polyListType);
                }
            }
            ...
            // exactly 2 qualifiers needed to bind to
            if (qualifiers.getQualifier().size() == 0) {
                QualifierType qualifier1 = objectFactoryECC.createQualifierType();
                qualifier1.setUri(Util.DEFAULT_NO_SELECTION_ENTRY);
                qualifiers.getQualifier().add(qualifier1); // 1
            }
            if (qualifiers.getQualifier().size() == 1) {
                QualifierType qualifier2 = objectFactoryECC.createQualifierType();
                qualifier2.setUri(Util.DEFAULT_NO_SELECTION_ENTRY);
                qualifiers.getQualifier().add(qualifier2); // 2
            }
            // remove any qualifier besides of the first two (a loaded list may contain these)
            List<QualifierType> qualifier = qualifiers.getQualifier();
            while (qualifier.size() > 2) {
                qualifier.remove(2);
            }
        } catch (Exception ex) {
