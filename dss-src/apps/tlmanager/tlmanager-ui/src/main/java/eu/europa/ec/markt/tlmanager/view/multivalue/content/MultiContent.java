...
public abstract class MultiContent {
    ...
    private static final Logger LOG = Logger.getLogger(MultiContent.class.getName());

    private MultivalueModel multiValueModel;
     ...
     */
    public MultiContent() {
        multiValueModel = new DefaultMultivalueModel(); // which is doing nothing
    }
     ...
     */
    protected abstract Object retrieveComponentValue(boolean clearOnExit);
    ...
    public void updateContent(String key) {
        if (!justOpened) {
            Object componentValue = retrieveComponentValue(false);
    ...
    public void setValue(String key, Object value) {
        LOG.log(Level.FINE, "Set for item {0} the value {1} in {2}", new Object[] { key, value, this });
    ...
    public Object getValue(String key) {
        LOG.log(Level.FINE, "Get item value for {0} in {1}", new Object[] { key, this });
    ...
    public String retrieveContentInformation() {
        ...
        int size = multiValueModel.getValues().size();
        if (size == 1) {
            entries = entry;
        }
        return size + " " + entries;
     ...
     */
    public void setMultiValueModel(MultivalueModel multiValueModel) {
