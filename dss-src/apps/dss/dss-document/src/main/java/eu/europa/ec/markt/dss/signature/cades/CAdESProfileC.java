...
public class CAdESProfileC extends CAdESProfileT {
     ...
     */
    private OtherCertID makeOtherCertID(X509Certificate cert) throws NoSuchAlgorithmException,
    ...
    private CrlValidatedID makeCrlValidatedID(X509CRL crl) throws NoSuchAlgorithmException, CRLException {
        ...
        if (crl.getExtensionValue("2.5.29.20") != null) {
            ...
            crlid = new CrlIdentifier(new X500Name(crl.getIssuerX500Principal().getName()), new DERUTCTime(
                    crl.getThisUpdate()), crlnumber);
        } else {
            crlid = new CrlIdentifier(new X500Name(crl.getIssuerX500Principal().getName()), new DERUTCTime(
                    crl.getThisUpdate()));
        }
     ...
     * @throws IOException
     */
    private OcspResponsesID makeOcspResponsesID(BasicOCSPResp ocspResp) throws NoSuchAlgorithmException,
    ...
    private Hashtable<ASN1ObjectIdentifier, ASN1Encodable> extendUnsignedAttributes(
        ...
        ValidationContext validationContext = certificateVerifier.validateCertificate(signingCertificate,
                ...
                new CompositeCertificateSource(new ListCertificateSource(parameters.getCertificateChain()),
                        optionalCertificateSource), null, null);
        ...
        try {
            ...
            unsignedAttrs.put(PKCSObjectIdentifiers.id_aa_ets_certificateRefs, new Attribute(
                    PKCSObjectIdentifiers.id_aa_ets_certificateRefs, new DERSet(new DERSequence(
                            completeCertificateRefs.toArray(otherCertIDArray)))));
            unsignedAttrs.put(PKCSObjectIdentifiers.id_aa_ets_revocationRefs, new Attribute(
                    PKCSObjectIdentifiers.id_aa_ets_revocationRefs, new DERSet(new DERSequence(
                            completeRevocationRefs.toArray(crlOcspRefArray)))));
