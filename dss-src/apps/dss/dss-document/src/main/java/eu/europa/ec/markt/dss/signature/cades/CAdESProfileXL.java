    ...
    private Hashtable<ASN1ObjectIdentifier, ASN1Encodable> extendUnsignedAttributes(
        ...
        try {
            ...
            for (CRL relatedcrl : validationContext.getNeededCRL()) {
                crlValues.add(new CertificateList((ASN1Sequence) ASN1Object.fromByteArray(((X509CRL) relatedcrl)
                        .getEncoded())));
            }

            for (BasicOCSPResp relatedocspresp : validationContext.getNeededOCSPResp()) {
                ocspValues.add((new BasicOCSPResponse((ASN1Sequence) ASN1Object.fromByteArray(relatedocspresp
                        .getEncoded()))));
            }
            ...
            unsignedAttrs.put(PKCSObjectIdentifiers.id_aa_ets_revocationValues, new Attribute(
                    PKCSObjectIdentifiers.id_aa_ets_revocationValues, new DERSet(revocationValues)));
            ...
            unsignedAttrs.put(PKCSObjectIdentifiers.id_aa_ets_certValues,
                    new Attribute(PKCSObjectIdentifiers.id_aa_ets_certValues, new DERSet(new DERSequence(
                            certificateValues.toArray(certValuesArray)))));
