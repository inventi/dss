...
class PdfStamperImp extends PdfWriter {
    ...
    void close(HashMap moreInfo) throws IOException {
        ...
        if (altMetadata != null) {
            // ARHS: change start
        	// ARHS: prevent changes that tamper the PDF validation
        	// ORIGINAL: PdfStream xmp;
        	// ORIGINAL: try {
        	// ORIGINAL: 	XmpReader xmpr = new XmpReader(altMetadata);
        	// ORIGINAL: 	if (!xmpr.replace("http://ns.adobe.com/pdf/1.3/", "Producer", producer))
        	// ORIGINAL: 		xmpr.add("rdf:Description", "http://ns.adobe.com/pdf/1.3/", "pdf:Producer", producer);
        	// ORIGINAL: 	if (!xmpr.replace("http://ns.adobe.com/xap/1.0/", "ModifyDate", date.getW3CDate()))
        	// ORIGINAL: 		xmpr.add("rdf:Description", "http://ns.adobe.com/xap/1.0/", "xmp:ModifyDate", date.getW3CDate());
        	// ORIGINAL: 	xmpr.replace("http://ns.adobe.com/xap/1.0/", "MetadataDate", date.getW3CDate());
            // ORIGINAL: 	xmp = new PdfStream(xmpr.serializeDoc());
        	// ORIGINAL: }
        	// ORIGINAL: catch(SAXException e) {
        	// ORIGINAL: 	xmp = new PdfStream(altMetadata);
        	// ORIGINAL: }
        	// ORIGINAL: catch(IOException e) {
        	// ORIGINAL: 	xmp = new PdfStream(altMetadata);
        	// ORIGINAL: }
        	// ORIGINAL: xmp.put(PdfName.TYPE, PdfName.METADATA);
        	// ORIGINAL: xmp.put(PdfName.SUBTYPE, PdfName.XML);
        	// ORIGINAL: if (crypto != null && !crypto.isMetadataEncrypted()) {
        	// ORIGINAL: 	PdfArray ar = new PdfArray();
        	// ORIGINAL: 	ar.add(PdfName.CRYPT);
        	// ORIGINAL: 	xmp.put(PdfName.FILTER, ar);
        	// ORIGINAL: }
        	// ORIGINAL: if (append && xmpo != null) {
        	// ORIGINAL: 	body.add(xmp, xmpo.getIndRef());
        	// ORIGINAL: }
        	// ORIGINAL: else {
        	// ORIGINAL: 	catalog.put(PdfName.METADATA, body.add(xmp).getIndirectReference());
        	// ORIGINAL: 	markUsed(catalog);
        	// ORIGINAL: }
            // ARHS: change end
        }
        try {
        ...
        // ARHS: change start
        // ARHS: prevent changes that tamper the PDF validation
        // ORIGINAL: newInfo.put(PdfName.MODDATE, date);
        // ARHS: change end
