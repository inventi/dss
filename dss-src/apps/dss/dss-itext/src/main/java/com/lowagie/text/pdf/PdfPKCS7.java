...
/*
 * The original code is copyrighted as in the original comment block further below.
 *
 * The changes as applied by the developer (see above) are marked with:
 * // ARHS: change start
 * // ARHS: [an optional comment]
 * // ARHS: change end
 *
 * The original code is commented out and marked (in between the above markers) with:
 * // ORIGINAL:
 *
 * @version $Revision: 1871 $ - $Date: 2013-04-08 14:38:37 +0200 (Mon, 08 Apr 2013) $
 */
...
public class PdfPKCS7 {
    ...
    // ARHS: change end
    private static final Logger LOG = Logger.getLogger(PdfPKCS7.class.getName());
    // ARHS: change start
    ...
    private void findOcsp(ASN1Sequence seq) throws IOException {
        ...
        // ARHS: change start
        // ORIGINAL: BasicOCSPResponse resp = BasicOCSPResponse.getInstance(inp.readObject());
        // ORIGINAL: basicResp = new BasicOCSPResp(resp);
        try {
    ...
    public PdfPKCS7(byte[] contentsKey, String provider) {
        try {
            ...
            // ARHS: change start
            // ORIGINAL: if (rsaData.size() > 1) {
            // ORIGINAL:     DEROctetString rsaDataContent = (DEROctetString)((DERTaggedObject)rsaData.getObjectAt(1)).getObject();
            // ORIGINAL:     RSAdata = rsaDataContent.getOctets();
            // ORIGINAL: }
            CMSSignedData rsaSignedData = null;
            ...
            if (rsaData.size() > 1) {
                ...
                try {
                    rsaSignedData = new CMSSignedData(contentsKey);
                    tsToken = new TimeStampToken(rsaSignedData);
                } catch (Exception ex) {
                    ex.printStackTrace();
                }
            }
            ...
            // ARHS: change start
            // ORIGINAL: digestAlgorithm = ((DERObjectIdentifier)((ASN1Sequence)signerInfo.getObjectAt(2)).getObjectAt(0)).getId();
            if (tsToken == null) {
                digestAlgorithm = ((DERObjectIdentifier) ((ASN1Sequence) signerInfo.getObjectAt(2)).getObjectAt(0)).getId();
            } else {
                digestAlgorithm = tsToken.getTimeStampInfo().getMessageImprintAlgOID();
            }
            // ARHS: change end
            next = 3;
    ...
    public boolean verify() throws SignatureException {
        ...
        if (sigAttr != null) {
            ...
            if (RSAdata != null) {
                // ARHS: change start
                // ORIGINAL: byte msd[] = messageDigest.digest();
                // ORIGINAL: messageDigest.update(msd);
                LOG.log(Level.FINE, "RSA != null **************************************************************************************************************");
                // ARHS: change end
            }
            // ARHS: change start
            // ORIGINAL: verifyResult = (Arrays.equals(messageDigest.digest(), digestAttr) && sig.verify(digest));
            byte[] messageDigestValue = messageDigest.digest();
            if (LOG.isLoggable(Level.INFO)) {
                LOG.log(Level.INFO, "Size {0} -  {1}", new Object[] { messageDigestValue.length, digestAttr.length });
                LOG.log(Level.INFO, "+++++ {0} +++++", new String(Hex.encode(messageDigestValue)));
                LOG.log(Level.INFO, "+++++ {0} +++++", new String(Hex.encode(digestAttr)));
                LOG.log(Level.INFO, "Digest equality : {0} ", Arrays.equals(messageDigestValue, digestAttr));
            }
            boolean signatureMath = sig.verify(digest);
            LOG.log(Level.INFO, "Signature valid : {0} ", signatureMath);
            verifyResult = (Arrays.equals(messageDigestValue, digestAttr) && signatureMath);
            // ARHS: change end
        } else {
            if (RSAdata != null)
        ...
        // ARHS: change start
        LOG.log(Level.INFO, "Verify result {0}", verifyResult);
        // ARHS: change end
